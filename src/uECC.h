...
uECC_asm_small - Use GCC inline assembly for the target platform (if available),
optimized for minimum size. uECC_asm_fast  - Use GCC inline assembly optimized
for maximum speed. */
...
multiplication function. This will make things faster by about 8% but increases
the code size. */
...
Usage: Compute a hash of the data you wish to sign (SHA-2 is recommended) and
pass it in to this function along with your private key and a hash context.
...
/* uECC_bytes() function.
Returns the value of uECC_BYTES. Helpful for foreign-interfaces to higher-level
languages.
*/
int uECC_bytes(void);

/* uECC_curve() function.
Returns the value of uECC_CURVE. Helpful for foreign-interfaces to higher-level
languages.
*/
int uECC_curve(void);
